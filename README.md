...
把飞书云文档链接、导出的 Markdown、图片附件或完整 ZIP，转换成一个可在浏览器里继续编辑的 `xhs-studio.html`。默认只生成 HTML；确认排版后，再按需批量导出 1080 × 1440 PNG ZIP。
...
- **两遍走样式**：第一遍按 Markdown 符号照实转换，第二遍 AI 通读全文自行判断强调与结构（`==有色字==`、`++下划线++`、标题/卡片推断）；没有标记的纯文字稿也能自动补出结构。
...
- **连续分页**：正文、引用、卡片、代码块和单个序列项可按行跨页；单独一行 `---` 可强制换页。
- **总览连续编辑**：只保留横向总览，同时查看三张 3:4 页面，点击任意页即可在原位置编辑。
- **文字与样式**：加粗、有色字和下划线可叠加；H1、H2、引用、卡片、代码块和列表彼此互斥，可直接互切但不会嵌套套娃。
...
从飞书链接开始（需要先装好 `lark-cli` 并登录）：

```bash
node scripts/lark-export.js "https://xxx.feishu.cn/wiki/文档token" -o "/path/to/文章目录"
```

脚本会自己取文档、清洗飞书私有标签、逐张下载图片，产出标准 Markdown 包；未授权时会打印引导，按提示完成授权后重跑即可。
...
2. **两遍走样式**：第一遍按 Markdown 符号照实转换；第二遍 AI 通读全文自行判断——关键词句用 `==有色字==`、`++下划线++` 或加粗标出，没有结构标记的原稿先补出标题、列表、卡片等结构再转。合适才标，不过度标记。
3. 缺少副标题时，AI 根据全文补一句简短总结。
4. 让用户选择全封面、半封面或无封面。
5. 全封面生成前确认最终标题和副标题，图片规格为 1080×1440；半封面图片规格为 1080×720；无封面不生成图片。
6. 未指定画风时：全封面默认生成“主题配图 + 已确认标题/副标题”；半封面默认生成无文字主视觉，标题和副标题留在 Studio 下半页编辑。仅用户明确要求，或文章是工具流程/品牌介绍时，半封面才使用“左字右图”的横幅构图。
7. 用户希望加入 IP 时，可询问是否提供参考图；未提供时按主题生成。来源/去向 Logo 仅用于明确的转换流程、工具介绍或用户点名要求。
8. 需要 AI 配图时先检查当前会话是否具备生图能力；不可用则如实说明，并改用用户本地图、现有素材或先交付半封面/无封面的可编辑 HTML。
9. 生成 `xhs-studio.html`，返回本地文件路径和采用的封面形式、副标题。
10. 只有用户明确要求导出、验收或准备发布时，才导出 PNG ZIP。
...
| `- 项目` / `1. 项目` | 无序 / 有序列表 |
| 整段完整加粗且为 18–75 字，或以 `注意：` 等明确标签开头且为 10–75 字 | 卡片块（21 个标签词，角标自动推断） |
| `==文字==` | 有色字（行内强调色） |
| `++文字++` | 下划线（行内半高亮 + 底线） |
...
- 工具栏会根据当前光标或选区点亮已有样式。
- H1、H2、引用、卡片、代码块和列表只能直接互切；再次点击同一个按钮可取消回正文，不能叠加成嵌套块。
- 加粗、有色字和下划线只作用于选中文字，三者可同时叠加；再次点击其中一个只取消该样式。块样式互切时已有行内样式会继续保留。
- 表格内可按单个单元格编辑和应用加粗、有色字、下划线；跨单元格选区会提示缩小范围，一级/二级标题、引用、卡片、代码块和序列在单元格内也会被拦截，不会改坏表格结构。
...
- 双击图片可替换；单击选中后可删除。
- 拖动调整裁剪中心，滚轮控制缩放，边框控制图片框宽高；替换、裁剪、缩放和调框均可撤销。
