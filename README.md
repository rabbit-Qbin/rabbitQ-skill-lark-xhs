...
- **AI 封面工作流**：转换前询问是否需要封面；确认后可生成适配文章主题的封面，并通过 `--cover-image` 直接嵌入 Studio。
...
3. 询问是否需要 AI 封面，以及想要什么风格。
4. 用户只说“要封面”但没有指定风格时，默认生成适配文章主题的“主视觉图 + 标题 + 短副标题”。
5. 生成 `xhs-studio.html`，返回本地文件路径和采用的副标题。
...
- 封面支持“上图下文”和“关闭封面图后正文接入下半页”两种模式。
- 封面标题最多三行，副标题最多两行。
...
- 新增 `--cover-image`，支持在转换时直接嵌入本地封面图。
- Skill 会主动询问是否需要 AI 封面，并在用户确认后生成、嵌入到第 1 页。
- 默认仍只生成可编辑 HTML；PNG ZIP 只在用户明确需要时导出。
