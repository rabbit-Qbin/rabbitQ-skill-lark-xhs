...
![rabbitQ 飞书 Markdown 转小红书 3:4 图文 v0.9.5 工作流](assets/rabbitq-xhs-workflow-v095.svg)
...
- **三种封面形式**：全封面、半封面、无封面可随时互切；AI 生成的图片可通过 `--cover-image` 直接嵌入 Studio。
...
6. 使用固定人物或 IP 参考图时，保持服装、配饰、颜色和身体结构一致；未给参考图不擅自加入固定 IP。来源/去向 Logo 仅用于明确的转换流程、工具介绍或用户点名要求。
7. 生成 `xhs-studio.html`，返回本地文件路径和采用的封面形式、副标题。
8. 只有用户明确要求导出、验收或准备发布时，才导出 PNG ZIP。
...
- 明确 AI 封面的默认生成策略：全封面图文一体，半封面默认无文字主视觉，避免与 Studio 标题重复。
- 明确 IP 和 Logo 的使用边界：仅在提供参考或文章语境明确时使用，不作为通用默认装饰。
- 粘贴图片会按原图比例生成更合适的初始图片框，避免横向截图默认铺满正文后显得突兀。

## v0.9.4 更新

- 固定全封面图片为 1080×1440、半封面图片为 1080×720。
- 补充固定 IP 的一致性规则，禁止生成时自行改写服装、配饰和身体结构。
- Studio 封面形式提示直接显示对应图片规格。
