...
- **结构化转换**：识别 H1/H2、普通正文、加粗、引用、无序列表、有序列表、GFM/HTML 表格和本地图片。
- **连续分页**：正文按行跨页；`---` 手动硬断点；结构块与长表格按行规则保持完整。
- **封面两种模式**：上图下文，或关闭封面图后用上半页标题、下半页接续正文。
- **可编辑 Studio**：修改文字、加粗、有色字、下划线、卡片、引用、序列和表格单元格。
...
- `一级标题` / `二级标题` / `引用块` / `卡片` / `序列`：选中文字后应用对应块样式。光标在已有块里再点**同一个**按钮 → 取消回正文；点**另一个**样式按钮 → 直接互切（例如引用变卡片、卡片变一级标题）。
- `有色字` / `下划线`：使用当前强调色；再点一次取消。
