...
![rabbitQ 飞书 Markdown 转小红书 3:4 图文 v0.9.4 工作流](assets/rabbitq-xhs-workflow-v094.svg)
...
- **图片编辑**：可在光标处插图，也可替换、删除、裁剪、缩放、调尺寸、上下移动、跨页拖动和并排。
...
3. 让用户选择全封面、半封面或无封面。
4. 全封面生成前确认最终标题和副标题，图片规格为 1080×1440；半封面图片规格为 1080×720，默认生成无文字主视觉图；无封面不生成图片。
5. 使用固定人物或 IP 参考图时，保持服装、配饰、颜色和身体结构一致，不自行增加或改写穿搭。
6. 生成 `xhs-studio.html`，返回本地文件路径和采用的封面形式、副标题。
7. 只有用户明确要求导出、验收或准备发布时，才导出 PNG ZIP。
