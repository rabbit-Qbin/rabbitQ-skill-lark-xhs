...
![rabbitQ 飞书 Markdown 转小红书 3:4 图文 v0.9.3 工作流](assets/rabbitq-xhs-workflow-v093.svg)
...
3. 让用户选择全封面、半封面或无封面。
4. 全封面生成前确认最终标题和副标题；半封面默认生成无文字主视觉图；无封面不生成图片。
5. 生成 `xhs-studio.html`，返回本地文件路径和采用的封面形式、副标题。
6. 只有用户明确要求导出、验收或准备发布时，才导出 PNG ZIP。
...
- 新增全封面、半封面、无封面三态和 `--cover-mode` 参数。
- 全封面独占首张 3:4 图片；半封面保留可编辑标题；无封面让正文接入首张下半页。
- Skill 会先让用户选择封面形式；全封面生成前必须确认最终标题与副标题。
