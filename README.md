...
- 列表按钮只作用于光标所在项，不再整组取消或切换。
- 连续有序列表由全文流统一编号，自动分页后仍跨页顺延。
- 列表项按整项分页，页尾空间不足时整体移到下一页。
- “保存编辑 HTML”中的主动删改会原样保留，升级排版时不再误判为正文损坏。

## v0.9.7 更新

- 修复图片控制点的拖拽光标：横向、纵向和右下角缩放分别显示正确方向；点击其它区域会退出图片选中状态。
- 修复有序/无序列表反复切换时标记写入正文、重复叠加的“套娃”问题。
- 删空列表正文后保留当前空列表项，后续编号自然顺延；只在空项行首 Backspace 时退出列表。
- 连续有序列表即使被自动分页切开，也会跨页持续编号；只有真正的内容块或手动分页才会重新起号。

## v0.9.6 更新

- 生成封面前会自检图像生成能力；不可用时明确给出本地图、现有素材或无图 HTML 的替代路径。
- 用户需要 IP 时才询问是否提供参考图，不将固定角色或穿搭规则写死在默认流程中。
- GitHub 工作流图改为无版本号的固定图片，避免文档视觉信息过期。
