...
- **AI 封面直插**：转换时可通过 `--cover-image` 把本地 PNG/JPG/WebP 直接嵌入为第 1 页封面；配合图像生成能力，生成后无需手工替换。
- **可编辑 Studio**：工具栏以 H1 / H2 / B / A / U 等符号呈现并同步点亮当前样式，有序与无序列表使用独立按钮。
- **总览连续编辑**：只保留横向 3:4 总览；点击任意页即可在原位置编辑，正文保存为一条连续流，分页只负责显示与导出。
...
- **光标插图**：顶部图片按钮可把本地图片插入当前正文光标位置，正文中间会自动拆成前后两段。
...
- 新增 `--cover-image`：本地 PNG、JPG/JPEG、WebP 或 GIF 可在转换时直接嵌入首张封面，不必打开 Studio 后再手动替换。
- Skill 转换流程会主动询问是否需要 AI 封面；确认后由 AI 生成并自动写入 Studio 封面。

## v0.9.1 更新

- 修复正文页尾空白处的临时光标在自动保存或重新分页后坍缩、闪烁两下就消失的问题。
- 在临时尾部光标处按一次 Enter 会转成且仅转成一条真实空行，继续输入、保存和跨页重排都不会丢失。

## v0.9.0 更新

- 删除“单页编辑”模式，只保留与小红书官方编辑器一致的横向总览；点击另一页即切换当前真实编辑画布。
- 正文状态新增唯一 `flowHtml` 连续流，3:4 `pages` 只作为派生视图和导出结果；旧版分页草稿仍可兼容读取。
- Enter、Backspace 等结构输入改为即时重排，不再等用户停下来后把多条空行成批推到下一页；页边界空段归属上一页，下一页首字保持顶格。
- 顶部工具栏新增“插入图片”，按正文光标位置插入并参与全文重新分页。
...
--cover-image <file>     本地 PNG/JPG/WebP/GIF，转换时直接嵌入首张封面
