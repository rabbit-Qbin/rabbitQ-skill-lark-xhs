...
- 正文先组成连续内容流，再按 3:4 画布实际高度分页。
- **手动分页**：源 Markdown 单独一行的 `---`（或 `***`、`___`）强制从新页顶部开始；与自动高度分页叠加，分页符不进导出图。勿用大量空行凑页码。细则见 `references/markdown-patterns.md`。
- 跨页重排后光标应回到原字符位置，不能跳到上一块或页首。
- 普通正文可按浏览器测量拆行跨页；一级/二级标题、引用、卡片、序列、短表格、图片块默认完整保留；长表格仅按完整数据行跨页。
- 封面关闭时，正文按源顺序填入封面下半页；第一个放不下的块起，后续顺延到下一页。
- 结构块后的光标锚点不占分页高度，但须能点击继续输入。
- 顶格空行须可删：在空行内或下一块开头 Backspace 清除连续顶格空行；一级标题编号前/标题文字开头均须生效，且重排后不得复活空行。
- 连续竖图可自动并排；用户拆分后尊重用户设置。编辑后由 Studio 内部自动重排，无单独「重新分页」按钮。
